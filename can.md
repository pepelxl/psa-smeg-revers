...
### ID 0x227, источник BSI, длина 5, частота 500мс  
пример: 00 50 11 30 00  
D3[5] временной интервал между автомобилями (статус для кнопки)  
...
D6[5] головное устройство перезагружается (индикация на приборной панели - идет загрузка (например, при смене темы)). Если вручную подать флаг, то головное устройство перезагружается на тему по умолчанию, но только если тема уже не по умолчанию (похоже на недоработку программистов SMEG)  
